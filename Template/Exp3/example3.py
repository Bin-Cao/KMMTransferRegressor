...
# P_S(y|x) = P_T(y|x)
# P_S(x) != P_T(x)
S_X = rng_S.normal(-2, 1.2, 30)[:, np.newaxis]
...
T_X = rng_T.normal(2, 1.2, 5)[:, np.newaxis]
...
# 5 target domian data
Ts_X = rng_Ts.normal(2, 1.2, 4)[:, np.newaxis]
Ts_y = (Ts_X[:, 0])**2+ 5 +  rng_Ts.normal(0, 2, Ts_X.shape[0])
